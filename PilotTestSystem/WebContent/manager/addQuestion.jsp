	...
	<script type="text/javascript">
			$(function(){
				$("#option1").click(function(){
						window.location.href="manager_addQuestion.jsp";
				});
			})
...
$(document).ready(function(){
	...
	<%  
		List<Topic>  topics = (List<Topic>)request.getAttribute("userlist"); 
			...
			<div class="welcome">
			    ...
			    <div class="search">
				    <form class="navbar-form navbar-left" role="search"
				    	action="<%=request.getContextPath() %>/ManagerSearchServlet?param=topic" method="post">
			    ...
			    <div class="table">
			    	<table class="table">
	                    ...
	                    <%for(Topic topic:topics){ %>
		                    ...
		                    <td ><%= topic.getId() %></td>
			                <td><%=topic.getChapterName() %></td>
			                <td><%=topic.getQuestion() %></td>
			                <td><%=topic.getType() %></td>
			                <td><%=topic.getAnswer() %></td>
			                <td><a href="DealQuestionServlet?delete_question_id=<%=topic.getId()%>">删除</a>丨<a href="DealQuestionServlet?update_question_id=<%=topic.getId()%>">更新</a></td>
			                </tr>
			                <%} %>
	                    </tbody>
                    </table>
			    ...
			    <div class="page">
				<ul class="pagination">
				    <li><a href="LoginServlet?page=<%=pg.getCurPage()-1%>">&laquo;</a></li>
				 	<% for(int i=0;i<pg.getPageCount();i++){ %>
				    <li><a href="LoginServlet?page=<%= i+1%>"><%= i+1%></a></li>
					<%} %>
				    <li><a href="LoginServlet?page=<%=pg.getCurPage()+1%>">&raquo;</a></li>
				</ul>
			    </div>
			</div>
		</div>
